...
#
#   PRODUCTION branches (write DIRECTLY to out/paper/<suite>/, with a provenance stamp):
#     suite1     Suite 1: rheology — Tresca baseline, elastic, DD-VM asym, DD-VM sym  → out/paper/set1_rheology
#     nd_sweep   Suite 2: in-plane N_D sweep at V=4                                    → out/paper/set2_nd_sweep
#     v_sweep    Suite 3: load V sweep (σ_Y=150)                                       → out/paper/set3_v_sweep
#     esweep     Appendix B: elastic E sweep at V=4                                    → out/paper/esweep
#     thickness  Suite 4: thickness sweep at matched deflection                        → out/paper/set4_thickness
#     convergence  SI Table S2 convergence of the baseline (own command, not in `all`)   → out/paper/convergence
...
# "DIAGNOSTIC / EXPLORATORY" banner is dev-history (writes to out/_sandbox), retained but not the paper set;
...
"""Run one M4 model at yield stress σY [Pa] and resolution (nx,nz); write to out/_sandbox/<name>.
...
function run_M4(; σY, nx, nz, name, nsteps = 22, springs = sp_one, gravity = true)
    outdir = joinpath(@__DIR__, "out", "_sandbox", name)
...
function run_tresca_deep(; σY, h, V, nx, nz, name, nsteps = 24, Nmem = 0.0, E = Emod, ramp_edge = true, moment = 0.0, shear_parabolic = true, load_uz = nothing, springs = sp_massless, follower = false, sym_V = nothing, sym_patch = 4.0e3, face_sxx = nothing, face_sxz = nothing, outroot = "_sandbox")
    outdir = joinpath(@__DIR__, "out", outroot, name)
    if isdir(outdir) && isfile(joinpath(outdir, "gpe_model.vtu"))
...
out/paper/set4_thickness/<name> (a STABLE, committed location — not _sandbox) and records the converged V
in tuned_V.txt so the hand-off is reproducible.  The h=60 member is the locked baseline
(set1_rheology/tresca_deep_150_60km_V4, V=4) and is NOT regenerated here."""
function run_tresca_thickness(; σY, h, w_target, V0, nx, nz, name, tol_m = 8.0, maxit = 6, nsteps = 24)
    outdir = joinpath(@__DIR__, "out", "paper", "set4_thickness", name)
    if isdir(outdir) && isfile(joinpath(outdir, "gpe_model.vtu"))
        @printf("%-22s exists — skipping (delete dir to force rerun)\n", name); return
    end
    mkpath(outdir)
...
each thickness.  Same platform as run_tresca_thickness.  Writes to out/paper/set4_thickness/vsweep/<name>
...
function run_tresca_thick_fixed(; σY, h, V, nx, nz, name, nsteps = 24)
    outdir = joinpath(@__DIR__, "out", "paper", "set4_thickness", "vsweep", name)
    if isdir(outdir) && isfile(joinpath(outdir, "gpe_model.vtu"))
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    # ========================= PRODUCTION (publication suites → out/paper/, provenance-stamped) =========================
        valid = join(PROD, " | ")
        ...
        nfail = Ref(0)
        for cmd in ARGS
        if cmd in ("suite1", "all")     # Suite 1: rheology — locked baseline σ_Y=150 (M_p=1.35), V=4
            run_tresca_deep(;  σY = 150e6,   h = 60e3, V = 4e12, nx = 800, nz = 48, outroot = "paper/set1_rheology", name = "tresca_deep_150_60km_V4")
            run_tresca_deep(;  σY = ELASTIC, h = 60e3, V = 4e12, nx = 800, nz = 48, outroot = "paper/set1_rheology", name = "elastic_deep_60km_V4")
            run_dd_vm_direct(; μ = 0.15, cohesion = 31.534e6, h = 60e3, V = 4e12, nx = 800, nz = 48, outroot = "paper/set1_rheology", name = "dd_vm_asym_60km_V4")
            run_dd_vm_direct(; μ = 0.35, cohesion = 36.695e6, h = 60e3, V = 4e12, nx = 800, nz = 48, symmetric = true, outroot = "paper/set1_rheology", name = "dd_vm_sym_60km_V4")
        end
        if cmd in ("nd_sweep", "all")   # Suite 2: in-plane N_D sweep at V=4 (±3; N_D=−4 buckles, +4 dropped for symmetry)
            for Nm in (-3.0, -2.0, -1.0, 1.0, 2.0, 3.0)
                try run_tresca_deep(; σY = 150e6, h = 60e3, V = 4e12, nx = 800, nz = 48, Nmem = Nm*1e12, outroot = "paper/set2_nd_sweep", name = "tresca_deep_150_60km_V4_mem$(Int(round(Nm)))")
                catch e; nfail[] += 1; @printf("  N_D=%.0f FAILED: %s\n", Nm, sprint(showerror, e)); end
            end
        end
        if cmd in ("v_sweep", "all")    # Suite 3: load V sweep (σ_Y=150); the V=4 point is the suite1 baseline
            for Vt in (1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.5)
                vn = replace(replace(string(Vt), ".0" => ""), "." => "p")
                try run_tresca_deep(; σY = 150e6, h = 60e3, V = Vt*1e12, nx = 800, nz = 48, outroot = "paper/set3_v_sweep", name = "tresca_deep_150_60km_V$(vn)")
        ...
        if cmd in ("esweep", "all")     # Appendix B: elastic Young's-modulus sweep at V=4 (E×1 = elastic_deep_60km_V4 in suite1)
            for mul in (0.125, 0.25, 0.5, 2.0, 4.0, 8.0)
                ...
                try run_tresca_deep(; σY = ELASTIC, h = 60e3, V = 4e12, nx = 800, nz = 48, E = Emod*mul, outroot = "paper/esweep", name = "elastic_deep_60km_V4_E$(mn)")
        ...
        if cmd in ("thickness", "all")  # Suite 4: thickness sweep at matched trench deflection (h=60 baseline w=3233 m)
            for (hk, V0k) in ((30.0, 2.05), (40.0, 2.70), (50.0, 3.35))
                ...
                isfile(joinpath(@__DIR__, "out", "paper", "set4_thickness", nm, "gpe_model.vtu")) ||
        ...
        if cmd == "convergence"         # SI Table S2: mesh + load-increment convergence of the baseline (NOT in `all` — the 1200×72 run is expensive)
            for (nxc, nzc) in ((400, 24), (800, 48), (1200, 72))
                run_tresca_deep(; σY = 150e6, h = 60e3, V = 4e12, nx = nxc, nz = nzc, nsteps = 24, outroot = "paper/convergence", name = "bench_$(nxc)x$(nzc)")
            end
            for ns in (12, 48)
                run_tresca_deep(; σY = 150e6, h = 60e3, V = 4e12, nx = 800, nz = 48, nsteps = ns, outroot = "paper/convergence", name = "bench_800x48_ns$(ns)")
            end
        end
        end                                  # for cmd in ARGS
        if nfail[] > 0
