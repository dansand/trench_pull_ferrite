...
def main():
    ...
    ax[2].plot(par_n, zeta, color="#b2182b", lw=3.0, alpha=0.45, label=r"parabola $1-[(z-h/2)/(h/2)]^2$")
    ax[2].plot(tau_bn, zeta, color="#111", lw=1.4, ls="--", label="FE (idealized beam)")
    ax[2].axvline(0, color="0.8", lw=0.7); ax[2].set_ylim(1, -1)
    ax[2].set_xlabel(r"normalized shear  $\tau_{zx}/\tau_{zx}^{\max}$")
    ax[2].set_ylabel(r"normalized depth  $(z-h/2)/(h/2)$")
