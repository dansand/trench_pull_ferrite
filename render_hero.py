...
def pgrad(mod):
    ...
    return mod.grad_x_field(mod.array("sigma_xz [Pa]"))

# ===================== CONFIG (iterate here) =====================
MODEL_DIR = "out/paper/set1_rheology/tresca_deep_150_60km_V4"
...
OUT = "out/paper/figures/hero_tresca_deep60.png"
...
def load(model_dir, diff_dir=None):
    ...
    m = Model(model_dir)
    sxx, szz, sxz = (m.array("sigma_xx [Pa]"), m.array("sigma_zz [Pa]"), m.array("sigma_xz [Pa]"))
    f = dict(sxx=sxx, szz=szz, sxz=sxz, ux=m.array("u", 0), uy=m.array("u", 1))
    diffN, sxzN, rhoN = (sxx - szz), sxz, pgrad(m)
    m2 = None
    if diff_dir:
        m2 = Model(diff_dir)
        diffN = diffN - (m2.array("sigma_xx [Pa]") - m2.array("sigma_zz [Pa]"))
        sxzN = sxzN - m2.array("sigma_xz [Pa]")
        rhoN = rhoN - pgrad(m2)
    mesh = m.m
