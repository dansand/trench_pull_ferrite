...
| 3 | `hero_tresca_deep60.png` | `python scripts/render_hero.py` | `hero_tresca_deep60.csv` | `data/suite1_strength/tresca_deep_150_60km_V4` | `model/paper_models.jl suite1` |
...
| Table S3 | convergence table | `python scripts/render_convergence.py` | `convergence.csv` (+ `.md`; shipped copy is the record) | `data/convergence/` — **not included** | **run first:** `julia --project=. model/paper_models.jl convergence` |
...
arrays (`gpe_analysis.write_table`); each starts with a `#` header naming its script, figure and models. `reproduce.sh`
...
- the reconstruction errors (~2 % plate-top arm, ~8 % sea-level arm) are `tables/gpe_compare_reconstruction.csv`.
