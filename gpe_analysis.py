...
integral over the mesh nodes — no `sample_over_line` interpolation needed.
    ...
    mod = Model("out/vm_v76")
    szz = mod.array("sigma_zz [Pa]")        # (Nx, Nz) numpy array -> pcolormesh/imshow
...
class Model:
    ...
    def N(self):   return self.resultant("sigma_xx [Pa]")                              # axial force
    def V(self):   return self.resultant("sigma_xz [Pa]")                              # vertical shear resultant
    def GPE(self): return self.resultant("sigma_zz [Pa]")                              # RAW ∫σzz dz (σzz<0)
    ...
    def fx(self):  return self.integrate_z(self.array("sigma_xx [Pa]") - self.array("sigma_zz [Pa]"))  # N_D = ∫(σxx−σzz)dz
    def M(self):                                                                       # bending moment ∫σxx(z−h/2)dz
        return self.integrate_z(self.array("sigma_xx [Pa]") * (self.z - self.H / 2)[None, :])  # z = depth, arm about mid-plane
    ...
    def box_fields(self, rho_w=1000.0, rho_m=3300.0, g=9.81, datum=0.0, z_c=None, npts=500):
        ...
        szz = self.array("sigma_zz [Pa]"); sxx = self.array("sigma_xx [Pa]"); uz = self.array("u", 1)
        zdef = self.z[None, :] + uz
        if z_c is None:
...
def stress_depth_profiles(mod, line_keys=("moment_max", "shear_max", "outer_rise", "2·L1"),
    ...
    sxx = mod.array("sigma_xx [Pa]"); szz = mod.array("sigma_zz [Pa]"); sxz = mod.array("sigma_xz [Pa]")
