...
Uses the committed elastic baseline `set1_rheology/elastic_deep_60km_V4` (E=70 GPa, ν=0.25, h=60 km, Winkler
...
Prints the numbers for SI Text S1.

    /opt/anaconda3/envs/pyvista-env/bin/python render_benchmark.py
"""
...
m = Model("out/paper/set1_rheology/elastic_deep_60km_V4")

# FE profiles on the (undeformed, total-Lagrangian) mesh
...
mb = Model("out/idealized_beam_elastic")
Hb = mb.z.max(); zeta = (mb.z - Hb / 2) / (Hb / 2)
tau_b = mb.column(mb.array("sigma_xz [Pa]"), 0.5 * mb.xkm.max())   # clean mid-span column
...
OUT = "out/paper/figures/benchmark_boef.png"
